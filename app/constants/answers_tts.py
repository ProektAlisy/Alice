class AnswersTts:
    """Ответы пользователя с исправлениями интонаций."""

    def __setattr__(self, key, value):
        raise AttributeError("Messages is immutable")

    DONT_UNDERSTAND = (
        "Я н+е понимаю, что в+ы хотите сказать. Возмоожно, вы хотите "
        "использовать стандартные функции Алисы. Для этого, нужно выйти из "
        'навыка Собаки-помощники - сказав sil <[200]> "выйти из навыка" '
        "sil <[200]> или вы м+ожете воспользоваться спр+авкой, сказав  "
        'sil <[200]> "помощь"'
    )
    ...
    SMALL_GREETINGS = """
    Вы при+ехали к нам на обуч+ение, sil <[200]> чтобы получ+ить
    ...
    STANDART_ALICE_COMMAND = """Извините, ваша просьба отн+оситься к функциям
    Ал+исы. Чтобы Алиса могла Вам помочь,  выйдите из навыка, сказав "Выйти"
    и повторите просьбу алисе.""".replace(
        "\n    ",
        " ",
    )
